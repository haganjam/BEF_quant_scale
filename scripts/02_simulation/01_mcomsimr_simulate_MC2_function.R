...
#' @param start_abun vector of starting abundances for each species and each patch
...
simulate_MC2 <- function(patches, species, dispersal = 0.01, timesteps = 1200,
  ...
  if (length(start_abun) != (species*patches) ) {
    stop("add appropriate starting abundances")
  }
  
  dynamics.df <- data.frame()
  N <- t(matrix(start_abun, nrow = species, ncol = patches))
...
#' @title sim_metacomm_BEF()
#' 
#' @description Function to use the simulate_MC2 function to 
#' simulate BEF experiments
#' 
#' @param patches number of patches to simulate
#' @param species number of species to simulate
#' @param dispersal dispersal probability between 0 and 1
...
sim_metacomm_BEF <- function(species = 5, patches = 10,
  ...
  for (j in 1:species) {
    ...
    x <- simulate_MC2(species = 1, patches = patches, 
                      dispersal = dispersal, start_abun = rep(sum(start_abun)/patches, patches) ,
                      timesteps = timesteps,
                      extirp_prob = extirp_prob,
                      landscape = landscape, 
                      disp_mat = disp_mat, 
...
#' @title dispersal_matrix
#'
#' @description Generates dispersal matrix for metacommunity simulations 
...
#' @title env_generate()
#'
#' @description Generates density independent environmental conditions for 
#' metacommunity simulations code taken directly from: 
#' https://github.com/plthompson/mcomsimr
#'
#' @param landscape landscape generated by landscape_generate()
#' @param env.df optional dataframe with environmental conditions with columns: env1, patch, time
...
#' @title sim_start_abun()
#'
#' @description Function to to generate a random set of starting abundances
#' for a set of species and patches.
#'
...
sim_start_abun <- function(species, patches, min_SA, max_SA) {
  ...
  for(j in 1:patches) {
    ...
    if(length(z) != species) {
      stop("error, must be same number of starting abundances as species")
    }
