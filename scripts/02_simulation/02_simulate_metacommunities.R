...
# generate a random landscape: n1 - x, n2 - y
n1 <- 2
n2 <- 3

# check the dimensions match for the landscape construction
...
t1 <- round(seq(25, 25*2, length.out = n1), 0)
t2 <- round(seq(25, 25*3, length.out = n2), 0)
...
for(i in 1:N_REP) {
  ...
  autocorr <- round(runif(n = 1, min = 200, max = 500), 0)
  
  # use the env_generate() function to generate a landscape of environmental variables
  env_var <- env_generate(landscape = landscape_x, 
                          env1Scale = autocorr, 
                          timesteps = timesteps, 
                          plot = FALSE)
  ...
  # add to the list collecting the simaulations
  sim_list[[i]] <- output_list
  
}

# save the MC_sims object
saveRDS(object = sim_list, "results/mc_sim_list.rds")

# get an example dataset to work with for the dimensions
df_ex <- sim_list[[1]]$mc_dat
